...
import com.kdragon.bluestone.enums.MessageDestination;
import com.kdragon.bluestone.util.Paginator;
...
public class CoreCog extends Cog {
    ...
    public void cmdHelp(Context ctx) {
        ...
        if (ctx.args.length < 1) {
            for (com.kdragon.bluestone.Command cmd: new HashSet<>(bot.commands.values())) {
            ...
            for (int i = 0; i < ctx.args.length && i < 24; i++) {
                ...
                if (bot.commands.containsKey(litem)) {
                    com.kdragon.bluestone.Command cmd = bot.commands.get(litem);
