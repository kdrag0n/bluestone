...
public class TrackLoadHandler implements AudioLoadResultHandler {
    ...
    private final com.kdrag0n.bluestone.Context ctx;
    ...
    public TrackLoadHandler(com.kdrag0n.bluestone.Context ctx, boolean isPatron, AudioState state, AudioPlayerManager man,
    ...
    public void trackLoaded(AudioTrack track) {
        ...
        state.scheduler.queue(track, new com.kdrag0n.bluestone.voice.ExtraTrackInfo(ctx.channel, ctx.member));
        if (!state.scheduler.queue.isEmpty()) {
            AudioTrackInfo info = track.getInfo();

            if (canTalk) ctx.send(com.kdrag0n.bluestone.Emotes.getSuccess() + " Queued **" + info.title + "** by **" + info.author +
    ...
    public void playlistLoaded(AudioPlaylist playlist) {
        ...
        if (tracks.size() > mn) {
            if (canTalk) ctx.send(com.kdrag0n.bluestone.Emotes.getFailure() +
        ...
        if (canTalk) ctx.send(com.kdrag0n.bluestone.Emotes.getSuccess() + " Queued playlist **" + playlist.getName() + "**, length **" +
