...
public class GuildRoleOption {
    ...
    public boolean test(com.kdrag0n.bluestone.Context ctx, int mSend, boolean hasBeenMentioned, boolean hasMentionedOther) {
        ...
        if ((conditions & IS_BOT_OWNER) == IS_BOT_OWNER) {
            if (!com.kdrag0n.bluestone.Permissions.check(ctx, com.kdrag0n.bluestone.Permissions.BOT_OWNER))
                return false;
        }

        if ((conditions & IS_BOT_ADMIN) == IS_BOT_ADMIN) {
            if (!com.kdrag0n.bluestone.Permissions.check(ctx, com.kdrag0n.bluestone.Permissions.BOT_ADMIN))
                return false;
        }

        if ((conditions & IS_PATRON) == IS_PATRON) {
            return com.kdrag0n.bluestone.Permissions.check(ctx, com.kdrag0n.bluestone.Permissions.PATREON_SUPPORTER);
        }
