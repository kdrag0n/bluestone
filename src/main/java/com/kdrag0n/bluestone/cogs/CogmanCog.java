...
public class CogmanCog extends Cog {
    private static final Logger logger = LoggerFactory.getLogger(CogmanCog.class);
    ...
    private static final Pattern CLASS_PATH_PATTERN = Pattern
            .compile("^[a-z]+://?(?:[a-zA-Z0-9\\-_:]+/)+[a-zA-Z0-9\\-_.]+\\.jar/(?:[a-z0-9\\-_]+\\.)*[a-zA-Z0-9]+$");
    ...
    @Perm.Owner
    @Command(name = "cogall", desc = "Manage all the cogs - run on ALL shards.", aliases = {
            "cogsall" }, reportErrors = false)
    ...
    private Cog ensureLoaded(Context ctx, String path) {
        Optional<Cog> opt = bot.cogs.values().stream().filter(c -> c.getClass().getName().equals(path)).findFirst();
        if (opt.isPresent()) {
            return opt.get();
        } else {
    ...
    private void cmdList(Context ctx) {
        ...
        for (Class<? extends Cog> clazz : classes) {
            ...
            if (bot.cogs.values().stream().map(Cog::getClass).anyMatch(c -> c.getName().equals(clazz.getName())))
                result.append('*');
            else
                result.append(' ');
    ...
    private void cmdLoad(Context ctx) throws ReflectiveOperationException, MalformedURLException {
        ...
        if (CLASS_PATH_PATTERN.matcher(input).matches()) {
             ...
             * addURL.setAccessible(true); addURL.invoke(cl, new URL(uriPath));
             */
        ...
        if (bot.cogs.values().stream().anyMatch(c -> c.getClass().equals(clazz))) {
    ...
    private void cmdInfo(Context ctx) {
        ...
        EmbedBuilder emb = newEmbedWithAuthor(ctx).setColor(randomColor())
                .setTitle(cog.getCosmeticName() + " (" + cog.getName() + ')').setDescription(cog.getDescription());
