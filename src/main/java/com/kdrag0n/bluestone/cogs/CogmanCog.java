...
public class CogmanCog extends com.kdrag0n.bluestone.Cog {
    ...
    private static final String INVALID_LOAD = com.kdrag0n.bluestone.Emotes.getFailure() + " I need a valid class URI, path, or name!\n" +
            "\n" +
            "Examples:\n" +
            "```\n" +
    ...
    public CogmanCog(com.kdrag0n.bluestone.Bot bot) {
        super(bot);
    }
    ...
    @com.kdrag0n.bluestone.annotations.Command(name = "cogall", desc = "Manage all the cogs - run on ALL shards.", aliases = {"cogsall"},
            reportErrors = false)
    public void mainCmdAll(com.kdrag0n.bluestone.Context ctx) throws ReflectiveOperationException, MalformedURLException {
        for (com.kdrag0n.bluestone.Bot shard: bot.shardUtil.getShards()) {
    ...
    @com.kdrag0n.bluestone.Perm.Owner
    @com.kdrag0n.bluestone.annotations.Command(name = "cog", desc = "Manage all the cogs.", aliases = {"cogs"}, reportErrors = false)
    public void mainCmd(com.kdrag0n.bluestone.Context ctx) throws ReflectiveOperationException, MalformedURLException {
        if (ctx.args.empty) {
    ...
    private String getClassPath(com.kdrag0n.bluestone.Context ctx) {
        ...
        if (!com.kdrag0n.bluestone.util.Strings.isPackage(arg)) {
    ...
    private com.kdrag0n.bluestone.Cog ensureLoaded(com.kdrag0n.bluestone.Context ctx, String path) {
        Optional<com.kdrag0n.bluestone.Cog> opt = bot.cogs.values().stream()
    ...
    private void cmdList(com.kdrag0n.bluestone.Context ctx) {
        Reflections reflector = cogsReflector;

        if (ctx.args.length > 1 && com.kdrag0n.bluestone.util.Strings.isPackage(ctx.args.get(1)))
            reflector = new Reflections(ctx.args.get(0));

        Set<Class<? extends com.kdrag0n.bluestone.Cog>> classes = reflector.getSubTypesOf(com.kdrag0n.bluestone.Cog.class);
        StringBuilder result = new StringBuilder("```css\n");

        if (classes.size() < 1)
            result.append("Package contains no cogs! \uD83D\uDE22\n");

        for (Class<? extends com.kdrag0n.bluestone.Cog> clazz: classes) {
            result.append('[');

            if (bot.cogs.values().stream().map(com.kdrag0n.bluestone.Cog::getClass)
    ...
    private void cmdReload(com.kdrag0n.bluestone.Context ctx) throws ReflectiveOperationException, MalformedURLException {
        cmdUnload(ctx);
        cmdLoad(ctx);
    }

    private void cmdLoad(com.kdrag0n.bluestone.Context ctx) throws ReflectiveOperationException, MalformedURLException {
        if (ctx.args.length < 2) {
        ...
        } else if (com.kdrag0n.bluestone.util.Strings.isPackage(input)) {
            if (input.indexOf('.') == -1)
        ...
        com.kdrag0n.bluestone.Cog cog = (com.kdrag0n.bluestone.Cog) clazz.getConstructor(com.kdrag0n.bluestone.Bot.class).newInstance(bot);
        bot.registerCog(cog);
        cog.load();
    ...
    private void cmdUnload(com.kdrag0n.bluestone.Context ctx) {
        String path = getClassPath(ctx);
        com.kdrag0n.bluestone.Cog cog = ensureLoaded(ctx, path);

        bot.unregisterCog(cog);
        ctx.success("Cog `" + cog.getClass().getName() + "` unloaded.");
    }

    private void cmdEnable(com.kdrag0n.bluestone.Context ctx) {
        String path = getClassPath(ctx);

        ctx.fail("This feature hasn't been implemented yet.");
    }

    private void cmdDisable(com.kdrag0n.bluestone.Context ctx) {
        String path = getClassPath(ctx);

        ctx.fail("This feature hasn't been implemented yet.");
    }

    private void cmdInfo(com.kdrag0n.bluestone.Context ctx) {
        String path = getClassPath(ctx);
