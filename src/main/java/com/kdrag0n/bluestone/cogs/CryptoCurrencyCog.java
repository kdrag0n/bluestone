...
public class CryptoCurrencyCog extends com.kdrag0n.bluestone.Cog {
    ...
    public CryptoCurrencyCog(com.kdrag0n.bluestone.Bot bot) {
        super(bot);
    }
    ...
    private static void update() {
        try {
            Cryptocurrency[] data;
            try (ResponseBody rbody = com.kdrag0n.bluestone.Bot.http.newCall(new Request.Builder()
                    .get()
            ...
            for (Cryptocurrency currency: data) {
                ...
                newRanked.add(com.kdrag0n.bluestone.util.Strings.format("{0,number}. **{1}** ({2}) \u2022 **${3,number}** \u2022 {4}{5,number}% changed in last 24h",
    ...
    @com.kdrag0n.bluestone.annotations.Command(name = "convert", desc = "Convert between different currencies.", aliases = {"exchange"},
            usage = "[from currency] [to currency] [amount]")
    public void cmdConvert(com.kdrag0n.bluestone.Context ctx) {
        Cryptocurrency from;
        double amount;
        ...
        ctx.send(com.kdrag0n.bluestone.util.Strings.format("{0,number} {1} = **{2,number} {3}**", amount, from.symbol, converted, to)).queue();
    }
    ...
    private EmbedBuilder renderDetails(Cryptocurrency c) {
        ...
        return new EmbedBuilder()
                ...
                .addField("Rank", '#' + com.kdrag0n.bluestone.util.Strings.str(c.rank), false)
                .addField("Price", com.kdrag0n.bluestone.util.Strings.format("USD ${0,number}\nBTC Ƀ{1,number}\nEUR €{2,number}",
                ...
                .addField("Market Cap", com.kdrag0n.bluestone.util.Strings.format("USD ${0,number}\nEUR €{1,number}",
                        c.marketCapUSD, c.marketCapEUR), false)
                .addField("% Changed in Last Hour", com.kdrag0n.bluestone.util.Strings.format("{0,number}%", c.percentChange1h), true)
                .addField("% Changed in Last Day", com.kdrag0n.bluestone.util.Strings.format("{0,number}%", c.percentChange24h), true)
                .addField("% Changed in Last Week", com.kdrag0n.bluestone.util.Strings.format("{0,number}%", c.percentChange7d), true)
                .addField("Volume (last day)", com.kdrag0n.bluestone.util.Strings.format("USD ${0,number}\nEUR €{1,number}",
                        c.volume24hUSD, c.volume24hEUR), false);
    }

    @com.kdrag0n.bluestone.annotations.Command(name = "currencyinfo", desc = "Get detailed information about a cryptocurrency.",
            aliases = {"currency", "currency_info"}, usage = "[cryptocurrency symbol]")
    public void cmdCurrencyInfo(com.kdrag0n.bluestone.Context ctx) {
        Cryptocurrency c;
    ...
    @com.kdrag0n.bluestone.annotations.Command(name = "dcl", desc = "List all cryptocurrencies, with a detailed info page for each one.",
            usage = "{page #}", aliases = {"dcryptolist"})
    public void cmdDclFull(com.kdrag0n.bluestone.Context ctx) {
    ...
    @com.kdrag0n.bluestone.annotations.Command(name = "currencies", desc = "List all cryptocurrencies (paginated), sorted by market cap.",
            aliases = {"cryptolist", "cryptocurrencies", "currencylist", "currency_list"}, usage = "{page #}")
    public void cmdCurrencyList(com.kdrag0n.bluestone.Context ctx) throws IllegalAccessException {
        String name;
        Color color;
        if (ctx.guild == null) {
            color = Color.BLUE;
        ...
        if (ctx.args.length > 0) {
            if (com.kdrag0n.bluestone.util.Strings.is4Digits(ctx.args.get(0))) {
    ...
    @com.kdrag0n.bluestone.Perm.Admin
    @com.kdrag0n.bluestone.annotations.Command(name = "cryptoupdate", desc = "Update the cryptocurrency data.",
            thread = true, aliases = {"currencyupdate", "cupdate", "crypto_update"})
    public void cmdCryptoUpdate(com.kdrag0n.bluestone.Context ctx) {
        update();
