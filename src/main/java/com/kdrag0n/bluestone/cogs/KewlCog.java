...
public class KewlCog extends com.kdrag0n.bluestone.Cog {
    ...
    private final LoadingCache<User, byte[]> profileCache = CacheBuilder.newBuilder()
            ...
            .build(new CacheLoader<User, byte[]>() {
                ...
                public byte[] load(@Nonnull User user) throws Exception {
                    ResponseBody imgBody = com.kdrag0n.bluestone.Bot.http.newCall(new Request.Builder().get()
                    ...
                    // Flags
                    TIntList flags = com.kdrag0n.bluestone.enums.ProfileFlags.getFlags(bot, user);
                    ...
                    while (iterator.hasNext()) {
                        ...
                        switch (flag) {
                            case com.kdrag0n.bluestone.enums.ProfileFlags.BOT_OWNER:
                                iconStream = cl.getResourceAsStream("/assets/owner.png");
                                break;
                            case com.kdrag0n.bluestone.enums.ProfileFlags.BOT_ADMIN:
                                iconStream = cl.getResourceAsStream("/assets/key.png");
                                break;
                            case com.kdrag0n.bluestone.enums.ProfileFlags.PATREON_SUPPORTER:
                    ...
                    // render text
                    g2d.setColor(new Color(74, 144, 226, 255));
                    com.kdrag0n.bluestone.sql.UserProfile profile = profileDao.queryForId(user.getIdLong());
                    if (profile != null) {
                        try {
    ...
    private final Dao<com.kdrag0n.bluestone.sql.UserProfile, Long> profileDao;

    public KewlCog(com.kdrag0n.bluestone.Bot bot) {
        super(bot);

        profileDao = setupDao(com.kdrag0n.bluestone.sql.UserProfile.class);
    ...
    @com.kdrag0n.bluestone.annotations.Command(name = "profile", desc = "Display a user's profile.", usage = "[user / \"setup\" / \"bg\"]", thread = true)
    public void cmdProfile(com.kdrag0n.bluestone.Context ctx) throws SQLException {
        User user;
    ...
    @com.kdrag0n.bluestone.annotations.Command(name = "profilesetup", desc = "Set up your personal user profile.", thread = true)
    public void cmdProfileSetup(com.kdrag0n.bluestone.Context ctx) throws SQLException {
        ...
        try {
            ...
            com.kdrag0n.bluestone.sql.UserProfile profile = profileDao.queryForId(ctx.author.getIdLong());
            if (profile == null)
                profile = new com.kdrag0n.bluestone.sql.UserProfile(ctx.author.getIdLong(), 0, answers.toString());
            else
    ...
    @com.kdrag0n.bluestone.annotations.Command(name = "set_profile_bg", desc = "Set your profile background.",
            usage = "{\"reset\" or \"default\" to reset to default}", thread = true,
            aliases = {"profilebg", "profile_bg", "setprofilebg"})
    public void cmdSetProfileBg(com.kdrag0n.bluestone.Context ctx) {
        ...
        if (a.equalsIgnoreCase("reset") || a.equalsIgnoreCase("default")) {
            ...
            try (InputStream is = com.kdrag0n.bluestone.Bot.http.newCall(new Request.Builder()
                    .get()
                ...
                image = com.kdrag0n.bluestone.util.GraphicsUtils.resizeImage(image, PROFILE_WIDTH, PROFILE_HEIGHT);
    ...
    @com.kdrag0n.bluestone.annotations.Command(name = "profile_override_bg", desc = "Override an user's profile background. This just executes `profile bg` as them.",
            usage = "[@user/user ID] {to: reset/default / attach image}", thread = true)
    public void cmdProfileOverrideBg(com.kdrag0n.bluestone.Context ctx) {
        User target;

        if (ctx.args.length < 1) {
            ctx.fail("I need a @mention or user ID as first argument!");
            return;
        } else if (ctx.message.getMentionedUsers().size() > 0 && com.kdrag0n.bluestone.util.Strings.isMention(ctx.args.get(0))) {
            target = ctx.message.getMentionedUsers().get(0);
        } else if (com.kdrag0n.bluestone.util.Strings.isID(ctx.args.get(0))) {
    ...
    @com.kdrag0n.bluestone.annotations.Command(name = "profile_invalidate", desc = "Invalidate someone's profile in the cache.")
    public void cmdProfileInvalidate(com.kdrag0n.bluestone.Context ctx) {
        long target;
        ...
        }, e -> ctx.fail("Error retrieving user.\n```java" + com.kdrag0n.bluestone.util.StackUtil.renderStackTrace(e) + "```"));
    }
}
