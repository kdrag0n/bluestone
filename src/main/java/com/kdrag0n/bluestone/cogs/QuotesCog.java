...
import com.kdrag0n.bluestone.sql.Quote;
import com.kdrag0n.bluestone.sql.QuotesBannedMember;
...
public class QuotesCog extends com.kdrag0n.bluestone.Cog {
    ...
    public QuotesCog(com.kdrag0n.bluestone.Bot bot) {
        super(bot);
    ...
    @com.kdrag0n.bluestone.annotations.Command(name = "quote", desc = "Add, create, or view a quote!", thread = true,
            usage = "[action / id] {args?...}", aliases = {"quotes", "q"})
    public void cmdQuote(com.kdrag0n.bluestone.Context ctx) throws SQLException {
        if (ctx.args.empty) {
        ...
        else if (com.kdrag0n.bluestone.util.Strings.is4Digits(invoked))
            quoteShowId(ctx, Integer.parseInt(invoked));
        else if (com.kdrag0n.bluestone.util.Strings.isID(invoked)) {
    ...
    private void quoteCmdAdd(com.kdrag0n.bluestone.Context ctx) throws SQLException {
        if (ctx.rawArgs.length() < 8) {
            ...
            return;
        } else if (com.kdrag0n.bluestone.util.Strings.isID(ctx.args.get(1))) {
            quoteCmdAddMessage(ctx);
            return;
        }

        String text = com.kdrag0n.bluestone.util.Strings.renderMessage(ctx.message, ctx.guild, ctx.rawArgs.substring(ctx.args.get(0).length()).trim()
                .replace('\n', ' '));
        ...
        if (quotes >= 25 && !com.kdrag0n.bluestone.Permissions.check(ctx, com.kdrag0n.bluestone.Permissions.BOT_ADMIN) &&
    ...
    private void quoteCmdDelete(com.kdrag0n.bluestone.Context ctx) throws SQLException {
        ...
        if (quote == null) {
            ...
            return;
                ...
                ctx.author.getIdLong() != com.kdrag0n.bluestone.Bot.ownerId &&
    ...
    private void quoteCmdList(com.kdrag0n.bluestone.Context ctx) throws SQLException {
        ...
        if (ctx.args.length > 1) {
            if (com.kdrag0n.bluestone.util.Strings.is4Digits(ctx.args.get(1))) {
        ...
        if (ctx.guild == null) {
            ...
            color = NullValueWrapper.val(ctx.member.getColor()).or(Color.RED);
    ...
    private void quoteCmdRandom(com.kdrag0n.bluestone.Context ctx) throws SQLException {
    ...
    private void quoteCmdCount(com.kdrag0n.bluestone.Context ctx) throws SQLException {
        ctx.send("There are **" + dao.countOf() + "** quotes.").queue();
    }

    private void quoteCmdAddMessage(com.kdrag0n.bluestone.Context ctx) throws SQLException {
        final int i = ctx.flag ? 0 : 1;
        if (ctx.args.length < 2 || !com.kdrag0n.bluestone.util.Strings.isID(ctx.args.get(i))) {
        ...
        String text = com.kdrag0n.bluestone.util.Strings.renderMessage(msg, msg.getGuild(), msg.getContentRaw().replace('\n', ' '));
    ...
    @com.kdrag0n.bluestone.annotations.Command(name = "quote_ban", desc = "Ban an user from adding quotes.", usage = "[@mention/user ID]",
            thread = true, aliases = {"quotes_ban", "qb"})
    public void cmdQuoteBan(com.kdrag0n.bluestone.Context ctx) throws SQLException {
        long userId;
        User user;
    ...
    @com.kdrag0n.bluestone.Perm.Owner
    @com.kdrag0n.bluestone.annotations.Command(name = "quote_ban_list", desc = "List users banned from adding quotes.", thread = true,
            aliases = {"quotes_ban_list", "qb_list"})
    public void cmdQuoteBanList(com.kdrag0n.bluestone.Context ctx) throws SQLException {
        String rendered =
    ...
    @com.kdrag0n.bluestone.Perm.Owner
    @com.kdrag0n.bluestone.annotations.Command(name = "quote_ban_remove", desc = "Allow a banned user from adding quotes.", usage = "[@mention/user ID]",
            thread = true, aliases = {"quotes_ban_remove", "qb_remove", "qb_rm", "qb_del", "quote_ban_rm",
            "quote_ban_del", "quotes_ban_rm", "quotes_ban_del"})
    public void cmdQuoteBanRemove(com.kdrag0n.bluestone.Context ctx) throws SQLException {
        long userId;
        User user;
