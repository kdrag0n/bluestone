...
public class RolemanCog extends com.kdrag0n.bluestone.Cog {
    ...
    private final Dao<com.kdrag0n.bluestone.sql.GuildRoleOption, Long> dao;
    private final Dao<GuildMemberActions, Integer> profDao;

    public RolemanCog(com.kdrag0n.bluestone.Bot bot) {
        super(bot);

        dao = setupDao(com.kdrag0n.bluestone.sql.GuildRoleOption.class);
    ...
    @com.kdrag0n.bluestone.annotations.Command(name = "role", desc = "Get or remove a role.", aliases = {"rank"}, usage = "[role name]",
            thread = true, guildOnly = true)
    public void cmdRole(com.kdrag0n.bluestone.Context ctx) throws SQLException {
        if (ctx.args.empty || ctx.rawArgs.equals("list")) {
            List<com.kdrag0n.bluestone.sql.GuildRoleOption> options = dao.queryBuilder()
                    .where()
            ...
            if (options.size() > 0) {
                ...
                for (com.kdrag0n.bluestone.sql.GuildRoleOption option: options) {
                ...
                String h = ctx.member.hasPermission(Permission.MANAGE_ROLES) ?
                        ...
                        : "";
                ctx.send(com.kdrag0n.bluestone.Emotes.getFailure() + " This server doesn't have any roles available." + h).queue();
            }
        }
        ...
        com.kdrag0n.bluestone.sql.GuildRoleOption option = dao.queryForId(role.getIdLong());
        GuildMemberActions prof = profDao.queryBuilder()
                .where()
    ...
    @com.kdrag0n.bluestone.annotations.Command(name = "roles", desc = "Manage roles that users can get.", thread = true,
