...
public class RolemanCog extends Cog {
    ...
    private final LoadingCache<Long, Object> guildIsUsing = CacheBuilder.newBuilder().concurrencyLevel(6)
            .maximumSize(750).initialCapacity(25).expireAfterAccess(10, TimeUnit.MINUTES)
            .build(new CacheLoader<Long, Object>() {
                @Override
                public Object load(@Nonnull Long key) throws SQLException {
                    return dao.queryBuilder().where().eq("guildId", key).query().size() > 0 ? TRUE : FALSE;
                }
            });
    ...
    public void onGuildMessageReceived(GuildMessageReceivedEvent event) throws SQLException {
        ...
        if (guildIsUsing.getUnchecked(guildId) == TRUE) {
            ...
            GuildMemberActions prof = profDao.queryBuilder().where().eq("userId", userId).eq("guildId", guildId)
                    .queryForFirst();
            if (prof == null)
    ...
    public void cmdRole(Context ctx) throws SQLException {
        if (ctx.args.empty || ctx.rawArgs.equals("list")) {
            ...
            if (options.size() > 0) {
                ...
                for (GuildRoleOption option : options) {
                    ...
                    list.append("    \u2022 **").append(role.getName()).append("**\n");
                }

                Color c = ctx.guild.getSelfMember().getColor();

                ctx.send(new EmbedBuilder().setAuthor("Roles", null, ctx.jda.getSelfUser().getEffectiveAvatarUrl())
                        .setColor(c == null ? Color.WHITE : c).setDescription(list.toString()).build()).queue();
                return;
            } else {
        ...
        GuildMemberActions prof = profDao.queryBuilder().where().eq("userId", ctx.author.getIdLong())
                .eq("guildId", ctx.guild.getIdLong()).queryForFirst();
        if (prof == null) {
            prof = new GuildMemberActions(ctx.author.getIdLong(), ctx.guild.getIdLong(), (short) 0, false, false);
            profDao.create(prof);
        }
