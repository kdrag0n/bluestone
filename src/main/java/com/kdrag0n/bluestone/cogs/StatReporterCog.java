...
public class StatReporterCog extends com.kdrag0n.bluestone.Cog {
    ...
    public StatReporterCog(com.kdrag0n.bluestone.Bot bot) {
        super(bot);
        ...
        if (bot.getShardNum() == 1 && bot.getConfig().has("graphite_host") &&
            ...
            com.kdrag0n.bluestone.Bot.scheduledExecutor.scheduleAtFixedRate(this::graphiteReport, 2, 60, TimeUnit.SECONDS);
        }
    }
    ...
    private void reportCarbonitex(String key) {
        com.kdrag0n.bluestone.Bot.http.newCall(new Request.Builder()
                .post(new FormBody.Builder()
                        .add("key", key)
                        .add("servercount", Strings.str(bot.shardUtil.getGuildCount()))
                        .build())
                .url(CARBONITEX)
                .build()).enqueue(com.kdrag0n.bluestone.Bot.callback(response -> {
            if (!response.isSuccessful()) {
